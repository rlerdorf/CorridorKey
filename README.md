...
1.  **The Original RGB Image:** The to-be-processed green or blue screen footage. The model works in the sRGB color gamut (interchangeable with REC709 gamut), but the engine can ingest a range of input color spaces and decode them to scene-linear for you (see below).
...
The model expects sRGB display-referred input, so camera-native plates (ARRI LogC, Sony S-Log3, RED Log3G10, ACES, Rec.2020…) must be decoded first — feeding log/wide-gamut data straight in produces flat, wrong colors and a degraded key. Pass `--color-space` to declare the source encoding:

```bash
# ARRI Alexa 35 (LogC4, EI-independent)
uv run corridorkey run-inference --color-space arri-logc4

# Sony S-Log3, or let CorridorKey read it from the file's metadata
uv run corridorkey run-inference --color-space slog3
uv run corridorkey run-inference --color-space auto
```

Supported values: `srgb` (default), `linear`, `rec709`, `rec2020`, `arri-logc3` (use `--ei`/`--exposure-index` for the Exposure Index, default 800), `arri-logc4`, `slog3`, `redlog3g10`, `acescct`, `aces2065-1`. `auto` reads color metadata from video headers (ffprobe) and EXR headers; because camera log encodings are frequently *untagged*, `auto` falls back to `srgb` with a warning — set `--color-space` explicitly when that happens. The legacy `--linear`/`--srgb` flags still work as aliases. Output passes are unchanged (FG in sRGB, Matte/Processed in linear EXR).
...
4.  **Configure:** Once your clips have both Inputs and AlphaHints, select "Process Ready Clips". The wizard will prompt you to configure the run:
    *   **Input color space:** Tell the engine your sequence's color space (sRGB, linear, ARRI LogC, S-Log3, etc.). Non-sRGB encodings are decoded to scene-linear before inference.
